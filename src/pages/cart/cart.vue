<template>
  <div class="cart">
    <order-header title="我的购物车" tips="温馨提示：产品是否购买成功，以最终下单为准哦，请尽快结算！"/>
    <div class="wrapper">
      <div class="container">
        <div class="cart-box">
          ...
          <ul class="cart-item-list">
            <li class="cart-item" v-for="(item,index) in cartList" v-bind:key="index">
              <div class="item-check">
                <span class="checkbox" v-bind:class="{'checked':item.productSelected}" @click="updateCart(item,'*')">
                </span>
              </div>
              <div class="item-name">
                <img v-bind:src="item.productMainImage" alt="">
              ...
              <div class="item-stock">{{ item.productStock }}件</div>
...
export default {
  name: "cart",
  components: {HomeServiceBar, IndexNavFooter, OrderHeader},
  data() {
    return {
      cartList: [],//商品列表
  ...
  methods: {
    ...
    // 购物车下单
    order() {
      let isEmpty = this.cartList.every(item => !item.productSelected);
      if (isEmpty) {
        this.$message.warning('请选择一件商品');
      } else {
        this.$router.push('/order/orderConfirmation');
      }
    }
  }
}
